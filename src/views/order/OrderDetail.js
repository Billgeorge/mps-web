...
export default function OrderDetail(props) {
  ...
  const callBackSuccessGetProduct = (product) => {
    let orderProductItems = []
    if (order.orderItemDTOs && order.orderItemDTOs.length > 0) {      
      for (let orderItem of order.orderItemDTOs) {
        orderProductItems.push({
          productName: orderItem.productDTO.name,
          attrs: orderItem.productDTO.attributes,
    ...
    setProducts(orderProductItems)
  }

  const getProductDetail = () => {
    let url = `${CORE_BASEURL}/dropshippingsale/public/view/checkout/${order.productId}`
    consumeServiceGet(callBackError, callBackSuccessGetProduct, url)
  }
            ...
            <Grid container className={classes.boxDetail} spacing={3}>
              {
                ...
                (<Grid item key={orderItem.productName} ><span className={classes.valueTextDetail}>{orderItem.productName}</span> <br />
                  <span className={classes.valueTextDetail}>Cantidad:{orderItem.quantity} </span><br />
                  <span className={classes.valueTextDetail}>Atributos:{orderItem.attrs}</span><br />
                  <span className={classes.valueTextDetail}>Sku:{orderItem.sku}</span><br />
                </Grid>)
                )
              }
            </Grid>
