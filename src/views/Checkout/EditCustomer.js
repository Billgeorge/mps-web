...
export default function EditCustomer(props) {
    ...
    React.useEffect(() => { updateCities() }, [citiesResponse, customer]);
    const updateCities = () => {
        if (initialLoadDone) {
            return
        }
        console.log("update cities")
        let newCities = citiesResponse.filter(record => record.state.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase()
            == customer.department.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase())
